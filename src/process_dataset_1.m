...
save('train_set_mnist_35_150feats','features','labels');
...
[features,labels] = extract_pos(img,targets);
save('test_set_mnist_35_150feats','features','labels');
disp('');
...
for i = 1:size(samples,1)
    ...
    if(length(feat_one_sample) >= numfeat)
        ...
        train_features(i,:) = [feat_one_sample, zeros(1,numfeat-length(feat_one_sample))];
    end
end
